...
#'
#' @param data data frame with retention time named X(min), intensity named Y(Counts),
#'        batch and Compound. If a column named group as factor is existed, it is
#'        used as group, otherwise the group is combined with batch and Compound.
...
geom_chromat <- function(data, legend.position = "none",
  ...
  if("group" %in% colnames(data) & class(data$group) == "factor"){
  ...
  if(shift){
    ...
    if(any(class(label) == "data.frame")){
      ...
      p <- p +
        ggrepel::geom_text_repel(data = data.label, max.overlaps = max.over,
                                 aes(x = `X(min)`, y = `Y(Counts)`,label = label),
                                 show.legend=FALSE)
    }else if(label){
      data.label <- data %>%
        group_by(group) %>%
